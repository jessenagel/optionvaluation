...
#include <math.h>
#include <time.h>
double chebyshev(int n, double x,double* c){ //Method for recursively calculating chebyshev polynomials
...
double findminimum(double* V,int n){ //Method for finding mininum in array
    double min = V[0];
    for(int i =1;i<n;i++){
        if(min > V[i]){
            min = V[i];
...
double findmaximum(double* V,int n){ //Method for finding maximum in array
    double max = V[0];
    for(int i =1;i<n;i++){
        if(max < V[i]){
            max = V[i];
...
double* solve(int n,double** A,double* B,int* lambda){ //Back substitution
...
double* Gauss(int n,double** A,double* B,int* lambda){ //Gauss Elimination with Scaled Partial Pivitong
...
double* fit(double* X, double* Y,int m){ //Least squares regression using chebyshev polynomials
...
double normal2(){ //Random draws from N(0,1) using Box-Muller Transformation
...
int main(int argc, char **argv) {
    ...
    int M = 100000; //Amount of simulation paths, j in loops
    int N = 50 * T ;  //Amount of Points in discrete time, i in loops
    double phi_alt[M];
    double** S;
