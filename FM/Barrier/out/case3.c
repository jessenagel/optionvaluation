...
int main(int argc, char **argv) {
    ...
    for(int i=0; i < N ;i++){
        ...
        for (int j=0; j < M; j++){
            ...
            double d1_H = (log((H / K)) + (Theta) * (r - q + (sigma * sigma) / 2)) / (sigma * Theta_sqrt);
            double d2_H = d1_H - sigma * Theta_sqrt;


            double e_qt = exp(-q * (Theta));
            double P = phi(-d2) * K * exp(-r * Theta) - S * e_qt * phi(-d1) ;
            double P_H = phi(-d2_H) * K * exp(-r * Theta) - H * e_qt * phi(-d1_H) ;

            double P_accent = e_qt * (phi(d1)-1);
            double P_accent_H = e_qt * (phi(d1_H)-1);

            double S_roof = (Q_1 * (K - P)) / (Q_1 - (1 + P_accent));
            double H_roof = (Q_1 * (K - P_H)) / (Q_1 - (1 + P_accent_H));
            if(H > H_roof){//region A
                if(S<H){
                    V = V + exp(-r * t) * (K - H);
                    break;
                }
