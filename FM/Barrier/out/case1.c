...
//    double exponentfirstpart = exp((r - q - (sigma * sigma) / 2) * delta_t);
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < M; j++) {
            ...
            double W_1 = (-1 + e_qt * phi(-d1) + pow((H / S), 2 * lambda) * (e_qt * phi(-d1_H_2) - (2 * lambda - 2) * p_0_H_2 * (S / (H * H)))) /
                         (beta_minus * pow(S, (beta_minus - 1)) - beta_plus * pow(H, (beta_minus - beta_plus)) * pow(S, (beta_plus - 1)));

            double S_roof_1 = K - p_1 - W_1 * (pow(S, beta_minus) - pow(H, (beta_minus - beta_plus)) * pow(S, beta_plus));
